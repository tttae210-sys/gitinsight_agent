...
HINT_STYLES = [
    {
        ...
        "approach": "정답을 직접 말하지 말고, '왜 그렇게 했는지', '이 상황에서 어떤 문제가 생길 수 있는지' 등 연속적인 반문 형식으로 힌트를 제공하세요.",
        "tone": "차분하고 논리적인 어조로, 지원자가 스스로 사고를 확장할 수 있도록 유도하세요."
    },
    {
        "persona": "실무 경험을 바탕으로 비슷한 실패 사례를 들어주는 시니어 개발자",
        "approach": "실제 현업에서 비슷한 코드 패턴으로 장애가 났던 상황을 짧게 비유로 들어주고, 지원자의 코드에서 그와 유사한 지점을 간접적으로 짚어주세요.",
        "tone": "친근하고 경험담을 나누는 어조로, 지원자가 공감할 수 있도록 이야기하듯 설명하세요."
    },
    {
        "persona": "핵심 키워드 하나만 던져주는 과묵한 기술 면접관",
        "approach": "힌트를 장황하게 설명하지 말고, 지원자가 검색하거나 떠올려야 할 핵심 기술 키워드나 개념어 1~2개만 짧게 언급하세요.",
        "tone": "간결하고 핵심만 짚는 어조로, 단답형에 가깝게 힌트를 제공하세요."
    },
    {
        "persona": "코드의 특정 줄을 지목하며 '이 부분을 다시 봐' 라고 안내하는 코드 리뷰어",
        "approach": "소스코드 문맥에서 힌트가 될 특정 라인을 자연어로 묘사하고, 그 라인이 어떤 문제와 연결될 수 있는지 간접적으로 암시하세요. 코드를 직접 인용하거나 코드 블록은 사용하지 마세요.",
        "tone": "코드 리뷰 피드백처럼 구체적이지만 정답은 말하지 않는 어조로 작성하세요."
    },
    {
        "persona": "반대 상황을 가정해 역질문을 던지는 창의적 면접관",
        "approach": "'만약 이 코드가 트래픽이 100배 늘어난다면?', '만약 이 함수가 동시에 1000번 호출된다면?' 처럼 극단적 가정 상황을 제시하고, 지원자 스스로 문제를 발견하게 유도하세요.",
...
def extract_interview_question(state: InterviewState) -> dict:
    ...
    if next_step == "HINT":
        ...
        hint_system_msg = (
            ...
            "2. 정답을 직접 말하지 마세요.\n"
            "3. 힌트는 한 가지만 제공하고, 3~5문장 이내로 간결하게 작성하세요.\n"
